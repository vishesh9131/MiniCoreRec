...
def get_labels():
    try:
        ...
        file_path = os.path.join(BASE_DIR, "data", "ML", "Data", "labelele.csv")
...
def predict():
    try:
        ...
        model_path = os.path.join(
            BASE_DIR, "data", "ML", "Model", f"{data['model']}.pth"
        )
        adj_matrix = np.loadtxt(
            os.path.join(BASE_DIR, "data", "ML", "Data", "adj.csv"), delimiter=","
        )
        wgt_matrix = np.loadtxt(
            os.path.join(BASE_DIR, data, "ML", "Data", "label.csv"), delimiter=","
        )
        df = pd.read_csv(os.path.join(BASE_DIR, "data", "ML", "Data", "labelele.csv"))
        col = df.values.flatten()
        ...
        model.eval()
        node_index = list(node_labels.keys())[
            list(node_labels.values()).index(data["label"])
        ]
        recommended_indices = cs.predict(
            model,
            adj_matrix,
            node_index,
            top_k=data["topK"],
        ...
        recommended_labels = [
            node_labels.get(idx, "Label not found") for idx in recommended_indices
        ]
